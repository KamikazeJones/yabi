...
#define x_add(STACK) { cpu->accu += cpu->memory[cpu->STACK + cpu->STACK ## P - 1]; }
#define x_sub(STACK) { cpu->accu -= cpu->memory[cpu->STACK + cpu-> STACK ## P - 1]; }
#define x_clear { cpu->accu = 0; }
#define x_drop(STACK) { -- cpu->STACK ## P; }

#define x_and(STACK) {cpu->accu &= cpu->memory[cpu->STACK + cpu->STACK ## P - 1]; }
#define x_or(STACK)  {cpu->accu |= cpu->memory[cpu->STACK + cpu->STACK ## P - 1]; }
#define x_xor(STACK) {cpu->accu ^= cpu->memory[cpu->STACK + cpu->STACK ## P - 1]; }

#define x_push(STACK) { cpu->memory[cpu->STACK + cpu->STACK ## P ++] = cpu->accu; }
#define x_pop(STACK) { cpu->accu = cpu->memory[cpu->STACK + --cpu->STACK ## P]; }
...
BCM *run(char *prog, int patch, int debug, int mem_size, int s_stack_address, int r_stack_address, int prog_start) {
    ...
    while( (bc = *(cpu->memory+cpu->pc)) !='H' ) {
        ...
        switch(bc) {
            ...
            case 'T': cpu->SP-=4; cpu->accu = x_load(cpu, cpu->S+cpu->SP); break;
            case 't': x_store(cpu, cpu->accu, cpu->S+cpu->SP); cpu->SP+=4; break;
